...
class CancelOrderRequest(BaseModel): 
  order_id: Optional[str] = Field(description='''Reya-assigned order ID to cancel, as returned by `CreateOrderResponse`. Provide `orderId`, or a non-zero `clientOrderId` when `orderId` is absent. If both are supplied, `orderId` takes precedence and `clientOrderId` is ignored.''', default=None, alias='''orderId''')
