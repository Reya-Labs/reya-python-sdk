...
class ModifyOrderRequest(BaseModel):
    ...
    is_buy: StrictBool = Field(description="Order side. Immutable — restate the resting order's value. Combined with `qty`, sets the signed `OrderDetails.quantity` (int256). A mismatch is rejected with `MODIFY_IMMUTABLE_MISMATCH`.", alias="isBuy")
    ...
    reduce_only: Optional[StrictBool] = Field(default=None, description="On-chain `OrderDetails.reduceOnly`. Immutable — restate the resting order's value. A mismatch is rejected with `MODIFY_IMMUTABLE_MISMATCH`.", alias="reduceOnly")
