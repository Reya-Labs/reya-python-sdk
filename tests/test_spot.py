...
async def test_spot_mass_cancel(reya_tester: ReyaTester):
    ...
    logger.info("=" * 80)
    logger.info(f"SPOT MASS CANCEL TEST: {symbol}")
    logger.info("=" * 80)
    ...
    for i in range(num_orders):
        ...
        buy_price = round(reference_price * price_factor, 4)  # Round to 4 decimals (price spacing 0.0001)
    ...
    open_order_ids = [o.order_id for o in open_orders if o.symbol == symbol]
    ...
    open_order_ids_after = [o.order_id for o in open_orders_after if o.symbol == symbol]
