...
class CreateOrderResponse(BaseModel): 
  ...
  cancel_reason: Optional[CancelReason] = Field(description='''Why an order reached a terminal `CANCELLED` status. Present only when `status` is `CANCELLED`. `NO_LIQUIDITY` (IOC found no resting liquidity) and `IOC_REMAINDER` (IOC partially filled, remainder cancelled) are returned on `createOrder` / `modifyOrder` responses; `GTT_EXPIRED`, `USER_CANCEL`, `MASS_CANCEL` and `CANCEL_ALL_AFTER` are delivered on the `walletOrderChanges` stream. `SELF_TRADE_PREVENTION` (the order would have matched your own resting order) cancels the **aggressor (taker) only** — your resting order is left untouched, so there is no resting-side STP event. It appears on the response when a self-crossing IOC or modify cancels the taker, and on the `walletOrderChanges` stream when a self-crossing GTC aggressor is cancelled.''', default=None, alias='''cancelReason''')
