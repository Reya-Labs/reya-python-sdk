...
class ModifyOrderRequest(BaseModel):
    ...
    client_order_id: Optional[StrictStr] = Field(default=None, description="Restated client-provided order ID, as a decimal string (`uint64`). Used as the lookup key only when `orderId` is absent, and then it must be non-zero. If the resting order has a non-zero client id, this field must be restated and must match it; omit only when the resting order has no client id. The modification cannot assign a new `clientOrderId`.", alias="clientOrderId")
