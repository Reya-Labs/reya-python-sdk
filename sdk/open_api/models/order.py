...
class Order(BaseModel):
    """
    Order state returned by openOrders, orderChanges, and orderHistory. Event streams and history can contain multiple updates for the same order. Field descriptions specify snapshot/event availability; see OrderStatus for lifecycle states.
    """ # noqa: E501
    ...
    avg_fill_px: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, alias="avgFillPx")
    first_fill_id: Optional[StrictStr] = Field(default=None, description="Identifier of the first fill this update represents. Together with fillCount it identifies the fills as a contiguous ID range [firstFillId, firstFillId + fillCount - 1]. For a taker update, the first fill of its matching round; for a maker update, its single fill. Present only on fill updates; absent for non-fill updates and resting-order snapshots.", alias="firstFillId")
    ...
    triggered: Optional[StrictBool] = Field(default=None, description="For protective stops, false means armed and true means fired; never true for LIMIT orders. Fired children are cancel-only. Omitted on orderHistory, where absence means unknown: use openOrders or walletOrderChanges to distinguish phases. On those two surfaces, omission on older deployments can be treated as false. See POST /v2/createOrder for firing and child-order behavior.")
    ...
    __properties: ClassVar[List[str]] = ["exchangeId", "symbol", "accountId", "orderId", "sequenceNumber", "clientOrderId", "qty", "execQty", "cumQty", "avgFillPx", "firstFillId", "fillCount", "side", "limitPx", "orderType", "triggerPx", "triggered", "timeInForce", "expiresAfter", "reduceOnly", "postOnly", "status", "createdAt", "lastUpdateAt", "cancelReason", "cancelReasonMessage"]
    ...
    @field_validator('avg_fill_px')
    def avg_fill_px_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value
