...
class CreateOrderResponse(BaseModel): 
  ...
  def unwrap_additional_properties(cls, data):
    ...
    known_object_properties = ['status', 'exec_qty', 'cum_qty', 'avg_fill_px', 'order_id', 'client_order_id', 'cancel_reason', 'cancel_reason_message', 'first_fill_id', 'fill_count', 'additional_properties']
    unknown_object_properties = [element for element in json_properties if element not in known_object_properties]
    # Ignore attempts that validate regular models, only when unknown input is used we add unwrap extensions
    if len(unknown_object_properties) == 0: 
      return data
  
    known_json_properties = ['status', 'execQty', 'cumQty', 'avgFillPx', 'orderId', 'clientOrderId', 'cancelReason', 'cancelReasonMessage', 'firstFillId', 'fillCount', 'additionalProperties']
