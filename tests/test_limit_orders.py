...
async def test_success_ioc(reya_tester: ReyaTester, test_qty, test_is_buy):
    ...
    try:
        ...
        # Validate
        await reya_tester.check_no_open_orders()
        order_execution_details = await reya_tester.check_order_execution(order_details)
...
async def test_success_ioc_with_any_market(reya_tester: ReyaTester, test_is_buy, test_symbol):
    ...
    try:
        # Get current prices to determine order parameters
        market_price = await reya_tester.get_current_price(symbol=test_symbol)
        await reya_tester.check_no_open_orders()

        market_config = await reya_tester.get_market_definition(symbol=test_symbol)
        ...
        if reya_tester.websocket:
            reya_tester.websocket.close()
        await reya_tester.close_exposure(test_symbol, fail_if_none=False)
...
async def test_failure_ioc_with_reduce_only_on_empty_position(reya_tester: ReyaTester):
    ...
    try:
        ...
        response = await reya_tester.create_limit_order(
            symbol=symbol,
            is_buy=test_is_buy,
            limit_px=str(price_with_offset),
            qty=str(test_qty),
        ...
        assert not response, "Order should not have been accepted with reduce_only flag on no position"
...
async def test_failure_ioc_with_input_validation(reya_tester: ReyaTester):
    ...
    try:
        ...
        for test_case in test_cases:
            ...
            await reya_tester.check_position_not_open(symbol)
            try:
                await reya_tester.create_limit_order(**test_case["params"], expect_error=True)
...
async def test_success_gtc_with_order_and_cancel(reya_tester: ReyaTester):
    ...
    try:
        ...
        buy_order_id = await reya_tester.create_limit_order(
            ...
            limit_px=order_details_buy.limit_px,
        ...
        await reya_tester.check_open_order_created(buy_order_id, order_details_buy)
        await reya_tester.check_position_not_open(symbol)
...
async def test_success_gtc_orders_with_execution(reya_tester: ReyaTester):
    ...
    try:
        ...
        logger.info(f"Created GTC BUY order with ID: {buy_order_id} at price {order_details_buy.limit_px}")
        ...
        await reya_tester.check_no_open_orders()
        order_execution_details = await reya_tester.check_order_execution(order_details_buy)
        await assert_position_changes(order_execution_details, reya_tester)
...
async def test_integration_gtc_with_market_execution(reya_tester: ReyaTester):
    ...
    try:
        ...
        sell_order_id = await reya_tester.create_limit_order(
            ...
            limit_px=order_details_sell.limit_px,
        ...
        if order_execution_details is not None and order_execution_details.timestamp > start_timestamp:
            ...
            if order_execution_details.side == Side.B:
                await assert_position_changes(order_execution_details, reya_tester)
                await reya_tester.check_order_execution(order_details_buy)
            else:
                await assert_position_changes(order_execution_details, reya_tester)
                await reya_tester.check_order_execution(order_details_sell)
            await reya_tester.check_order_not_open(buy_order_id)
            await reya_tester.check_order_not_open(sell_order_id)
        else:
            logger.info("Order was not filled")
            await reya_tester.check_open_order_created(buy_order_id, order_details_buy)
            await reya_tester.check_open_order_created(sell_order_id, order_details_sell)
