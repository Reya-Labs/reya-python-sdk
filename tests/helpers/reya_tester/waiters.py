...
class Waiters:
    ...
    async def for_order_execution(
        ...
        while time.time() - start_time < timeout:
            ...
            if (
                rest_position is None
                and position is not None
                and trade_seq_num is not None
                and trade_seq_num == position.last_trade_sequence_number
            ):
                elapsed_time = time.time() - start_time
    ...
    async def for_execution_bust(
        self,
        order_id: Optional[str] = None,
        timeout: int = 15,
    ) -> ExecutionBust:
        """Wait for an execution bust event via both WS and REST (spot or perp).
        ...
        while time.time() - start_time < timeout:
            ...
            if ws_bust is None:
                if order_id is not None:
                    ...
                    # Wait for any bust
                    if len(self._t.ws.execution_busts) > 0:
        ...
        raise RuntimeError(
            f"Execution bust not confirmed after {timeout}s, "
