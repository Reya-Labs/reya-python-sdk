...
not asset balances) and lives in ``tests/spot/test_balance_verification.py``
(exact per-side deltas + maker/taker conservation).
"""
...
async def test_maker_taker_match_e2e(
    ...
    # Step 5: WS order-changes event for maker — and when the maker reached
    # FILLED, the event must carry that terminal status (folded from
    # test_spot_ws_order_changes_on_fill when it was retired as a duplicate).
    ...
    ws_event = maker.ws.order_changes[maker_order_id]
    ws_status = ws_event.status.value if hasattr(ws_event.status, "value") else str(ws_event.status)
    assert ws_status in (
        "FILLED",
        "PARTIALLY_FILLED",
    ), f"[{market_type}] maker WS event must carry the fill status, got {ws_status}"
