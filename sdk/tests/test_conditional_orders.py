...
def assert_tp_sl_order_submission(
    order_details: Order, expected_order_details: OrderDetails, position: Position, reya_tester: ReyaTester
):
...
async def test_success_tp_order_create_cancel(reya_tester: ReyaTester):
    ...
    try:
        ...
        reya_tester.check_open_order_created(tp_order.order_id, tp_order_details)
        reya_tester.check_no_order_execution_since(active_tp_order.created_at)
        ...
        reya_tester.check_order_not_open(active_tp_order.order_id)
        reya_tester.check_no_order_execution_since(active_tp_order.created_at)
        reya_tester.check_position(
            symbol=tp_order_details.symbol,
            expected_qty=tp_order_details.qty,
...
async def test_success_sl_order_create_cancel(reya_tester: ReyaTester):
    ...
    try:
        ...
        reya_tester.check_open_order_created(order_response.order_id, sl_order_details)
        reya_tester.check_no_order_execution_since(active_sl_order.created_at)
        reya_tester.check_position(
            symbol=sl_order_details.symbol,
        ...
        reya_tester.check_order_not_open(active_sl_order.order_id)
        reya_tester.check_no_order_execution_since(active_sl_order.created_at)
        reya_tester.check_position(
            symbol=sl_order_details.symbol,
            expected_qty=sl_order_details.qty,
...
async def test_success_tp_wide_when_executed(reya_tester: ReyaTester):
    ...
    try:
        ...
        reya_tester.check_position_not_open(tp_order_details.symbol)
        reya_tester.check_order_not_open(tp_order.order_id)
        reya_tester.check_order_execution(tp_order_details)
...
async def test_success_sl_when_executed(reya_tester: ReyaTester):
    ...
    try:
        ...
        # wait 5 seconds for the order to propagate (skip when WS is implemented)
        await asyncio.sleep(5)
        reya_tester.check_position_not_open(sl_order_details.symbol)
        reya_tester.check_order_not_open(order_response.order_id)
        reya_tester.check_order_execution(sl_order_details)
...
async def test_failure_sltp_when_no_position(reya_tester: ReyaTester):
    ...
    try:
        ...
        reya_tester.check_no_open_orders()
        reya_tester.check_no_order_execution_since(start_time)
...
async def test_failure_cancel_when_order_is_not_found(reya_tester: ReyaTester):
    ...
    try:
        reya_tester.check_no_open_orders()
        try:
            reya_tester.client.cancel_order(order_id="unknown_id")
            # If we get here, the test failed - order should not have been accepted
            assert False, "No order to cancel, should have failed"
        except Exception as e:
            check_error_message(e, ["validation error"])
            # no code sent
            # check_error_code(e, "404")
