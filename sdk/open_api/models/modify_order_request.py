...
class ModifyOrderRequest(BaseModel):
    ...
    reduce_only: Optional[StrictBool] = Field(default=None, description="Signed OrderDetails.reduceOnly. Required and immutable on LIMIT modifications. Omit for STOP_LOSS/TAKE_PROFIT, including false: the SDK and server reconstruct the fixed signed value false; omission never inherits stored state.", alias="reduceOnly")
    limit_px: Annotated[str, Field(strict=True)] = Field(alias="limitPx")
    qty: Optional[Annotated[str, Field(strict=True)]] = None
    post_only: Optional[StrictBool] = Field(default=None, description="Post-modify maker-only flag, required for LIMIT. Omit for STOP_LOSS/TAKE_PROFIT, including false: the SDK and server reconstruct the fixed signed value false. A post-only LIMIT modification that would cross is rejected with POST_ONLY_WOULD_CROSS_ERROR and leaves the order unchanged.", alias="postOnly")
