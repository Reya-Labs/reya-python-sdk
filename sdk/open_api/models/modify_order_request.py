...
class ModifyOrderRequest(BaseModel):
    ...
    client_order_id: Optional[StrictStr] = Field(default=None, description="Restated client-provided order ID, as a decimal string (`uint64`). Used as the lookup key only when `orderId` is absent, and then it must be non-zero. If `orderId` is present, this field restates the resting order's immutable client id for signing; omit it when the resting order has no client id. `clientOrderId: \"0\"` is accepted only alongside `orderId` and is equivalent to omitting the field. The modification cannot assign a new `clientOrderId`.", alias="clientOrderId")
