...
balances) lives in ``tests/spot/test_ioc_orders.py`` and ``test_balance_verification.py``.
"""
...
@pytest.mark.asyncio
async def test_ioc_crosses_multiple_price_levels(
    market_config: SpotTestConfig | PerpTestConfig,
    market_type: str,
    maker: ReyaTester,
    taker: ReyaTester,
) -> None:
    """One IOC sized for two maker levels sweeps both in a single taker order.

    Previously pinned only on spot (tests/spot/test_ioc_orders.py) — on
    perp a multi-level sweep settles multiple fills from one taker order
    (batch fill path), which was untested.
    """
    await market_config.refresh_order_book(taker.data)
    await maker.orders.close_all(fail_if_none=False)
    await taker.orders.close_all(fail_if_none=False)
    ...
    if market_config.has_any_external_liquidity:
        pytest.skip("external liquidity present — multi-level sweep requires controlled book")

    lower_px = str(market_config.price(0.97))
    higher_px = str(market_config.price(0.99))
    qty_per_level = market_config.min_qty
    sweep_qty = str(Decimal(qty_per_level) * 2)

    level_ids = []
    for px in (lower_px, higher_px):
        params = LimitOrderParameters(
            symbol=market_config.symbol,
            is_buy=True,
            limit_px=px,
        ...
        assert order_id is not None, f"[{market_type}] expected order_id at {px}"
        await maker.wait.for_order_creation(order_id)
        level_ids.append(order_id)

    # Sell limit at the LOWER price crosses both bid levels; qty covers both.
    taker_params = LimitOrderParameters(
        symbol=market_config.symbol,
        is_buy=False,
        limit_px=lower_px,
    ...
    # Both maker levels consumed; the IOC did not rest.
    open_orders = await maker.client.get_open_orders()
    open_ids = {o.order_id for o in open_orders if o.symbol == market_config.symbol}
    for order_id in level_ids:
        assert order_id not in open_ids, f"[{market_type}] maker level {order_id} should be swept"

    open_orders_taker = await taker.client.get_open_orders()
    open_ids_taker = {o.order_id for o in open_orders_taker if o.symbol == market_config.symbol}
    assert taker_order_id not in open_ids_taker, f"[{market_type}] IOC must not rest after the sweep"
