...
class DepthSnapshot(BaseModel): 
  ...
  type: DepthSnapshotType = Field(description='''Literal discriminator for a bounded depth snapshot.''')
  bids: List[Level] = Field(description='''The fixed public WebSocket depth snapshot. This narrows the shared REST
  snapshot maximum from 1,000 to the 100 levels per side published by the
  WebSocket channel.
  ''')
  asks: List[Level] = Field(description='''The fixed public WebSocket depth snapshot. This narrows the shared REST
  snapshot maximum from 1,000 to the 100 levels per side published by the
  WebSocket channel.
  ''')
