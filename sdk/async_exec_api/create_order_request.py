...
class CreateOrderRequest(BaseModel): 
  ...
  post_only: Optional[bool] = Field(description='''Post-only (maker-only) intent for LIMIT GTC/GTT orders; rejected on IOC. Omit for STOP_LOSS/TAKE_PROFIT, including false: triggers sign OrderDetails.postOnly=false. A post-only LIMIT that would cross is rejected with POST_ONLY_WOULD_CROSS_ERROR.''', default=None, alias='''postOnly''')
  ...
  client_order_id: Optional[str] = Field(description='''Client-provided correlation ID, signed as OrderDetails.clientOrderId (uint64). Omit when unused; otherwise send a non-zero decimal string to preserve precision.''', default=None, alias='''clientOrderId''')
